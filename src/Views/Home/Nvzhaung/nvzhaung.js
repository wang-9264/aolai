...
class Nvzhaung extends Component {
    listClick(id) {
    ...
    state = {
      ...
      componentDidMount() {
        sessionStorage.getItem("homenvzhuang")
          ? this.setState({
              datalist: JSON.parse(sessionStorage.getItem("homenvzhuang"))
            })
          : axios
              .post(
                "https://cms.aolaigo.com/Handler/app_ActivityHandler.ashx",
                '{"os":"wap","opt":1,"cmd":1,"id":"7320"}'
              )
              .then(res => {
                sessionStorage.setItem("homenvzhuang", JSON.stringify(res.data.data));
                this.setState({
    ...
    render() {
        return (
            ...
            <div style={{background:'#f3f3f3',marginBottom:"0.7rem"}}>
                {/* 轮播 */}
                <Swiper mingzi="nvzhuangtop" configure={{pagination: {el: '.swiper-pagination',clickable: true,}}}>
...
export default withRouter(Nvzhaung)