...
class Nanzhuang extends Component {
    listClick(id) {
    ...
    state = {
      ...
      componentWillMount() {
        sessionStorage.getItem("homenanzhuang")
          ? this.setState({
              datalist: JSON.parse(sessionStorage.getItem("homenanzhuang"))
            })
          : axios
              .post(
                "https://cms.aolaigo.com/Handler/app_ActivityHandler.ashx",
                '{"os":"wap","opt":1,"cmd":1,"id":"7309"}'
              )
              .then(res => {
                sessionStorage.setItem("homenanzhuang", JSON.stringify(res.data.data));
                this.setState({
    ...
    render() {
        return (
            ...
            <div style={{background:'#f3f3f3',marginBottom:"0.7rem"}}>
                {/* 轮播 */}
                <Swiper mingzi="nanzhuangtop" configure={{pagination: {el: '.swiper-pagination',clickable: true,}}}>
...
export default withRouter(Nanzhuang)